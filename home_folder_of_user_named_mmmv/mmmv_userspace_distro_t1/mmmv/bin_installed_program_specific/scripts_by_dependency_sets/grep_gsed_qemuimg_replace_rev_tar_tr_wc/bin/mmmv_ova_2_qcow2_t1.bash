...
func_mmmv_bash_boilerplate_version(){
    local S_GUID="d40bde50-ca86-4e10-85cc-20d2e060b9e7"
...
func_mmmv_init_s_timestamp_if_not_inited_t1(){
    if [ "$S_TIMESTAMP" == "" ]; then
        if [ "`which date 2> /dev/null`" != "" ]; then
            ...
            echo ""
            echo "GUID=='28bc433d-6f98-404b-95cc-20d2e060b9e7'"
            echo ""
        fi
    fi
} # func_mmmv_init_s_timestamp_if_not_inited_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1(){
    ...
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo "GUID=='1f4f4432-f55a-410d-9bcc-20d2e060b9e7'"
        echo ""
    else
        if [ ! -e "$S_FP_ORIG" ]; then
            SB_S_FP_ORIG_VERIFICATION_FAILED="t"
            ...
            echo ""
            echo "GUID=='e865b04d-3fa1-493d-a5cc-20d2e060b9e7'"
            echo ""
        else
            if [ ! -d "$S_FP_ORIG" ]; then
                SB_S_FP_ORIG_VERIFICATION_FAILED="t"
                ...
                echo "    S_FP_ORIG==$S_FP_ORIG"
                echo ""
                echo "GUID=='01fef738-fd25-4024-85cc-20d2e060b9e7'"
                echo ""
            fi
        fi
    fi
    ...
    #        echo "GUID=='5426225a-44ca-4b15-91cc-20d2e060b9e7'"
    #        echo ""
    #    fi
} # func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t2(){
    if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo "GUID=='ee775444-69aa-4899-91bc-20d2e060b9e7'"
            echo ""
        else
            func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_assert_nonempty_string_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='272b645e-b8de-4743-a1bc-20d2e060b9e7'"
        echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then
            ...
            echo "GUID=='2da0ef53-3a6a-421d-a1bc-20d2e060b9e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='3533e1a3-7954-4962-81bc-20d2e060b9e7'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then
            ...
            echo ""
            echo "GUID=='21be7dd4-651a-4ec1-93bc-20d2e060b9e7'"
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then
        if [ "$S_IN" == "" ]; then
            ...
            echo "GUID=='2b41e9bb-90bc-442a-93bc-20d2e060b9e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
...
func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='517636d5-6735-4f89-83bc-20d2e060b9e7'"
        echo ""
        SB_NO_ERRORS_YET_1="f"
    fi
    #--------------------
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then
            echo "is expected to be initialized to \"t\" before calling this function."
            echo "GUID=='01b77e3f-01fe-438d-b5bc-20d2e060b9e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='24f097b2-8fbb-4128-b5bc-20d2e060b9e7'"
        fi
        if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then
            ...
            echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
            echo ""
            echo "GUID=='5db80574-b831-45a3-94bc-20d2e060b9e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then
        if [ "$SB_VARIABLE_VALUE" != "t" ]; then
            if [ "$SB_VARIABLE_VALUE" != "f" ]; then
                ...
                echo "GUID=='63f84739-cbb1-4209-84bc-20d2e060b9e7'"
                if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                    echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                fi
                echo ""
...
func_mmmv_report_an_error_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='2389c372-59d0-47ea-b4bc-20d2e060b9e7'"
        echo ""
    else
        echo ""
        echo -e "\e[31mThe code of this script is flawed.\e[39m"
        ...
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='0b48a036-a106-415a-95bc-20d2e060b9e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because
...
func_mmmv_report_missing_from_path_and_do_NOT_exit_t1() {
    ...
    if [ "$S_TMP_1" == "" ] ; then
        ...
        echo "GUID=='441689cd-b8bd-4602-82bc-20d2e060b9e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because
...
func_mmmv_verify_that_the_file_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='53e9c941-df3c-46ae-b1ac-20d2e060b9e7'"
        echo ""
        #--------
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='c277e1ec-a2d2-4881-af9c-20d2e060b9e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID=='2b819a1e-8deb-4574-959c-20d2e060b9e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='3e3be7d6-93a4-4dda-b29c-20d2e060b9e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='83655930-95d2-4c51-959c-20d2e060b9e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            ...
            echo "GUID=='51a0e51b-51b1-4f3a-848c-20d2e060b9e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "This function requires 2 parameters: S_FP, S_GUID_CANDIDATE"
        echo "and has an optional 3. parameter: SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE"
        echo "GUID=='771424ec-9fc1-4cdb-a18c-20d2e060b9e7'"
        echo ""
        #--------
        SB_VERIFICATION_FAILED="t"
    fi
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "Valid values are: \"t\", \"f\", \"\" ."
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='9480db45-b369-446d-a28c-20d2e060b9e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='3ce1945e-d55e-4de3-b27c-20d2e060b9e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo -e "\e[31mdoes not exist\e[39m."
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='eb15b95c-1495-44ad-827c-20d2e060b9e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ ! -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='58c3115c-da79-437c-a37c-20d2e060b9e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='ad168929-d3f6-4870-857c-20d2e060b9e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_add_bin_2_Z_PATH_and_optionally_share_man_2_MANPATH_t1(){
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_GUID_CANDIDATE" == "" ]; then
            ...
            echo "    S_GUID_CANDIDATE==\"\""
            echo ""
            echo "GUID=='60429d11-ec68-40f1-a46c-20d2e060b9e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_FP_INSTALLATION_FOLDER" == "" ]; then
            ...
            echo "GUID=='08568f3b-ee49-4a12-a56c-20d2e060b9e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
        ...
        if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "" ]; then
            ...
            if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "t" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "f" ]; then
                    ...
                    echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                    echo "GUID=='67433b53-c767-441a-a56c-20d2e060b9e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
            "$S_FP_INSTALLATION_FOLDER" "76e35f2d-a16b-47a9-a2cc-20d2e060b9e7"
        if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            #--------------
            func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                "$S_FP_INSTALLATION_FOLDER/bin" "53febc15-769a-4cae-a3cc-20d2e060b9e7"
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            ...
            if [ "$SB_MAN_FOLDER_OR_NONBROKEN_SYMLINK_TO_IT_EXISTS" == "f" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "t" ]; then
                    ...
                    func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                        "$S_FP_INSTALLATION_FOLDER/share/man" "25f8ccf2-e00d-4a0f-84cc-20d2e060b9e7"
                fi
            else
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='6a21c04c-4aea-401e-846c-20d2e060b9e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_assert_error_code_zero_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='429947d3-bb2e-4f0f-a15c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "GUID=='07b0fe58-4b25-40fc-b15c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_error_code_zero_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='a32cf02c-8e55-4c2d-935c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "Something went wrong. Error code: $S_ERR_CODE"
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='df8c8e2c-ce93-4561-935c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
...
func_mmmv_exc_verify_S_FP_ORIG_t1() {
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo ""
        echo "Aborting script."
        echo "GUID=='e1957b4c-f181-4e4b-a34c-20d2e060b9e7'"
        echo ""
    ...
    if [ ! -e "$S_FP_ORIG" ]; then
        ...
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='ed50a157-0b9c-434d-a34c-20d2e060b9e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    if [ ! -d "$S_FP_ORIG" ]; then
        ...
        echo "contains the script that prints this error message."
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='26185397-81d0-4d66-824c-20d2e060b9e7'"
        echo ""
        exit 1 # exit with an error
    fi
} # func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_exc_verify_S_FP_ORIG_t2(){
    if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo ""
            echo "has a domain of {\"f\", \"t\"}."
            echo "GUID=='f3d7703e-6cf3-425b-943c-20d2e060b9e7'"
            echo ""
        else
            func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_cd_S_FP_ORIG_and_exit_t1(){
    ...
    cd "$S_FP_ORIG"
    func_mmmv_assert_error_code_zero_t2 "$?" \
        "81d84e38-4b33-429c-b3cc-20d2e060b9e7"
    exit 0
} # func_mmmv_cd_S_FP_ORIG_and_exit_t1
...
func_mmmv_assert_nonempty_string_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='3a0dafa1-24cb-47d8-a53c-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='39d98804-65a8-43fb-813c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_IN" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be something other than an empty string."
        echo "GUID=='50f26542-a665-4604-b23c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_sbvar_domain_t_f_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='03bdc2a8-eaf4-455a-b12c-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='82c0da3c-54ad-4c87-812c-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$SB_VARIABLE_VALUE" != "t" ]; then
        if [ "$SB_VARIABLE_VALUE" != "f" ]; then
            ...
            echo ""
            echo "but it is expected to be either \"t\" or \"f\"."
            echo "GUID=='7c36b849-2bf3-4474-822c-20d2e060b9e7'"
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_assert_file_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='bf46e521-4849-4e19-b32c-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            ...
            echo "GUID=='bca9fb14-f347-4af7-a52c-20d2e060b9e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='0d031020-9b24-4d16-821c-20d2e060b9e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='5c39b73e-44f1-443e-841c-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='6852fd2c-4afe-4ca5-b11c-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='1490e451-2ae7-4424-910c-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='b40adb45-9608-40a2-a30c-20d2e060b9e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_assert_folder_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='109d8532-16d6-48f1-910c-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            echo -e "\e[31mThis code is flawed. \e[39m"
            echo "GUID=='1b38fd12-fc7f-4304-b50c-20d2e060b9e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='94f2a753-0faf-40ac-9d0c-20d2e060b9e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='56ef3f2f-0eef-491f-a5fb-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='74c7d41a-baf3-46af-b5fb-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ ! -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='e681fc4b-0979-4c2b-93fb-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='d2b0c93c-e5fc-4afd-82fb-20d2e060b9e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_exit_if_not_on_path_t2() { # S_COMMAND_NAME
    ...
    if [ "$S_LOCAL_VARIABLE" == "" ]; then
        ...
        echo "GUID=='c935f821-aee5-4abb-92eb-20d2e060b9e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_exc_exit_with_an_error_t1(){
    ...
    echo "GUID=='a59cdf35-505b-4f1f-b2eb-20d2e060b9e7'"
    echo ""
    cd "$S_FP_ORIG"
    exit 1 # exit with an error
} # func_mmmv_exc_exit_with_an_error_t1
...
func_mmmv_exc_exit_with_an_error_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='0c449c38-fc38-41ea-85eb-20d2e060b9e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    else
        ...
        echo "Aborting script."
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='597792aa-b1e5-46e4-84eb-20d2e060b9e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_determine_Awk_command_t1() { # prefers the GNU Awk to the BSD Awk
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "c4ded03e-a2bc-44a6-92bc-20d2e060b9e7"
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="67213b37-6aab-42a3-85bc-20d2e060b9e7"
        ...
        if [ "$SB_THROW" == "t" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='a3cf445a-7855-4830-92eb-20d2e060b9e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_determine_sed_command_t1() { # prefers the GNU sed to the BSD sed
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "98a07b28-4889-4c7b-b5bc-20d2e060b9e7"
        SB_AVOID_REINIT="$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED"
    fi
    #----------------------------------------------------------------------
    local SB_REINIT="t"
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="378f2761-b611-4bc6-b9bc-20d2e060b9e7"
        local S_FP_0="/tmp/this/proB9bably/do00es/noTt/eXXXist/ㄫ/$S_TMP_0"
        #------------------------------------------------------------------
        ...
        if [ "$SB_THROW" != "f" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='c65ddb32-aab9-4273-95eb-20d2e060b9e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_is_file_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='3be83be4-bdb3-4438-84eb-20d2e060b9e7'"
    ...
    if [ -d "$S_FP_CANDIDATE" ]; then
        ...
        echo "GUID=='311204e5-748d-43d3-97db-20d2e060b9e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_is_folder_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='cfcca021-ff88-4e3d-a2db-20d2e060b9e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
    if [ ! -d "$S_FP_CANDIDATE" ]; then
        ...
        echo "GUID=='e1e21878-c285-4a40-9ddb-20d2e060b9e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_create_folder_t1(){
    ...
    if [ "$S_FP_FOLDER" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='3f733d20-9907-4f20-a2db-20d2e060b9e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
    ...
    if [ -e "$S_FP_FOLDER" ]; then
        if [ ! -d "$S_FP_FOLDER" ]; then
            ...
            echo "Aborting script."
            echo "GUID=='52218b2b-b88e-42c0-85db-20d2e060b9e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1 # exit with an error
        fi
        ...
        wait # just in case
        if [ "$S_TMP_0" != "0" ]; then
            func_mmmv_exc_exit_with_an_error_t2 "2708c274-fd67-4320-b5bc-20d2e060b9e7" \
                "S_FP_FOLDER==$S_FP_FOLDER"
        fi
        if [ ! -e "$S_FP_FOLDER" ]; then
            func_mmmv_exc_exit_with_an_error_t2 "b1427358-8dac-4cb9-b3bc-20d2e060b9e7" \
...
func_mmmv_exc_assure_tmp_folder_existence_t1() {  # S_FP_TMP, S_GUID
    ...
    if [ "$S_GUID" == "" ]; then
        ...
        echo "GUID=='dc68225a-5285-4508-b4db-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP_TMP" ]; then
        if [ -h "$S_FP_TMP" ]; then
            ...
            func_mmmv_assert_error_code_zero_t1 \
                "$?" "af80072f-1cc8-4811-82bc-20d2e060b9e7"
            func_mmmv_wait_and_sync_t1
            if [ -h "$S_FP_TMP" ]; then
                echo ""
                echo "The path "
                ...
                echo "GUID==\"$S_GUID\""
                echo "GUID=='44878707-b9f7-4969-9cdb-20d2e060b9e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
        ...
        mkdir -p "$S_FP_TMP"
        func_mmmv_assert_error_code_zero_t1 \
            "$?" "1a43c63e-60c7-4621-82bc-20d2e060b9e7"
        func_mmmv_wait_and_sync_t1
        func_mmmv_assert_folder_exists_t1 "$S_FP_TMP" \
            '76724253-11cb-4840-82db-20d2e060b9e7'
    else
        if [ ! -d "$S_FP_TMP" ]; then
            echo ""
            if [ -h "$S_FP_TMP" ]; then
            ...
            echo "GUID==\"$S_GUID\""
            echo "GUID=='15b3caef-8a3a-48c1-82cb-20d2e060b9e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
...
func_mmmv_ln_create_hardlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ "$SB_THROW_ON_INVALID_DATA" == "" ]; then
        ...
        if [ "$SB_THROW_ON_INVALID_DATA" != "t" ]; then
            if [ "$SB_THROW_ON_INVALID_DATA" != "f" ]; then
                ...
                echo ""
                echo "Aborting script."
                echo "GUID=='1a2747b9-df25-427c-91cb-20d2e060b9e7'"
                echo ""
    ...
    if [ -e "$S_FP_LINK" ]; then
        if [ -d "$S_FP_LINK" ]; then # folder or a symlink to a folder
            if [ ! -h "$S_FP_LINK" ]; then # not a symlink, therefore a folder
                ...
                echo "    $S_FP_TARGET"
                echo ""
                echo "GUID=='2e5dc494-7588-40c5-b5cb-20d2e060b9e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            echo "GUID=='87b9d132-fba4-4dad-a4cb-20d2e060b9e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            if [ -h "$S_FP_LINK" ]; then # a broken symlink, therefore NOT missing
                ...
                echo "must not exist before the call to this function."
                echo "GUID=='152a6c11-7501-4f2d-a5cb-20d2e060b9e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ -e "$S_FP_TARGET" ]; then
            if [ -d "$S_FP_TARGET" ]; then # a folder or a symlink to a folder
                if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore a folder
                    ...
                    echo "    $S_FP_LINK"
                    echo ""
                    echo "GUID=='671bef2b-7540-4af8-b2cb-20d2e060b9e7'"
                    echo ""
                    if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                        exit 1
                    fi
            ...
            if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore missing
                ...
                echo "    $S_FP_LINK"
                echo ""
                echo "GUID=='3fb1752d-4e9e-47fe-83cb-20d2e060b9e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
    ...
    #----------------------------------------------------------------------
    local S_TMP_0="not_set_yet GUID=='0d97f67d-c5ab-47c8-91cb-20d2e060b9e7'"
    if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
        ...
        if [ "$S_TMP_0" != "0" ]; then
            ...
            echo "GUID=='181b9a24-5660-430d-8bbb-20d2e060b9e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            if [ -h "$S_FP_TARGET" ]; then
                ...
                if [ ! -h "$S_FP_LINK" ]; then
                    ...
                    echo "GUID=='d16dcc53-46bb-40af-a2bb-20d2e060b9e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -e "$S_FP_TARGET" ]; then # symlink to a folder or a file
                        if [ ! -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='2857f0e3-42f6-469d-b3bb-20d2e060b9e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ -d "$S_FP_TARGET" ]; then
                                if [ ! -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='2d014231-f66b-4a77-82bb-20d2e060b9e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='de9b7e1c-f1f5-453f-a2bb-20d2e060b9e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
                        ...
                        if [ -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='83a90837-7378-4ff4-83bb-20d2e060b9e7'"
                            echo ""
                            S_TMP_0="t"
                        fi
                    fi
                ...
                if [ ! -e "$S_FP_TARGET" ]; then # just an extra test
                    echo ""
                    echo "Problem detection branch marker "
                    echo "GUID=='a81ce92b-65e8-41ba-b5bb-20d2e060b9e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -d "$S_FP_TARGET" ]; then # just an extra test
                        echo ""
                        echo "Problem detection branch marker "
                        echo "GUID=='0d875826-f516-47f0-a5bb-20d2e060b9e7'"
                        echo ""
                        S_TMP_0="t"
                    else
                        if [ -h "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='e9fecb4a-af7a-47e1-91ab-20d2e060b9e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ ! -e "$S_FP_LINK" ]; then
                                echo ""
                                echo "Problem detection branch marker "
                                echo "GUID=='6bf35b46-bd1d-42a9-83ab-20d2e060b9e7'"
                                echo ""
                                S_TMP_0="t"
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='03228328-11bf-475a-a3ab-20d2e060b9e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
            ...
            if [ "$S_TMP_0" == "t" ]; then
                ...
                echo "GUID=='74f7a337-0d88-457a-82ab-20d2e060b9e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
                ...
                if [ "$S_TMP_0" != "f" ]; then
                    ...
                    echo "Aborting script."
                    echo "GUID=='dd807849-4ba3-48c2-95ab-20d2e060b9e7'"
                    echo ""
                    exit 1
                fi
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" != "t" ]; then
            ...
            echo -e "\e[31mThe implementation of this function is flawed. \e[39m"
            echo "Aborting script."
            echo "GUID=='13c33e5d-704b-4a15-91ab-20d2e060b9e7'"
            echo ""
            exit 1
        fi
    fi
...
func_mmmv_assert_environment_variable_set_t1() {
    ...
    if [ "$S_ENVIR_VALUE" == "" ]; then
        ...
        echo "GUID=='d468d92f-150a-4048-91ab-20d2e060b9e7'"
        echo ""
        exit 1 # exit with an error
    #else
...
func_mmmv_iterate_over_array_02n_t1() {
    ...
    if [ "$S_ARRAY_VARIABLE_NAME" == "" ]; then
        ...
        echo "GUID=='3791f034-7024-4c61-84ab-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_ITERATION_FUNCTION_NAME" == "" ]; then
        ...
        echo "GUID=='b6b10f38-c07a-4e27-a1ab-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
...
func_mmmv_include_bashfile_if_possible_t1(){ # S_FP_BASHFILE S_GUID_CANDIDATE SB_THROW_ON_ERROR
    ...
    if [ "$SB_THROW_ON_ERROR" == "" ]; then
        ...
        if [ "$SB_THROW_ON_ERROR" != "t" ]; then
            if [ "$SB_THROW_ON_ERROR" != "f" ]; then
                ...
                echo "GUID=='d98a0a4d-97d4-46a9-82ab-20d2e060b9e7'"
                echo ""
            fi
        fi
    fi
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='fa252e02-de53-46cd-b3ab-20d2e060b9e7'"
        echo ""
        if [ "$SB_THROW_ON_ERROR" == "t" ]; then
            exit 1
        fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='857de212-2ae3-4f7e-a3ab-20d2e060b9e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='12859bb5-28bf-4127-b5ab-20d2e060b9e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='4da8e2b1-0ccd-4ad2-97ab-20d2e060b9e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
...
func_mmmv_include_bashfile_if_possible_t2(){ # S_FP_BASHFILE S_GUID_CANDIDATE
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='4a1a22c2-3277-4f7e-b19b-20d2e060b9e7'"
        echo ""
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='33029e12-9f88-48e3-a29b-20d2e060b9e7'"
            echo ""
        fi
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='420a2e70-ea5e-4a82-a59b-20d2e060b9e7'"
            echo ""
        fi
    fi
    #----------------------------------------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='2082e391-5219-4849-8f9b-20d2e060b9e7'"
            echo ""
        fi
    fi
...
func_mmmv_assert_exists_on_path_t1() {
    ...
    if [ "$S_TMP_1" == "" ]; then
        ...
        echo "GUID=='af7a201f-973d-4cca-b59b-20d2e060b9e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_ln_create_or_overwrite_symlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ -e "$S_FP_LINK" ]; then
        if [ ! -h "$S_FP_LINK" ]; then # not a symlink
            ...
            echo ""
            echo "PWD==`pwd`"
            echo "GUID=='572bffc5-8037-42db-b29b-20d2e060b9e7'"
            echo ""
            exit 1 # exit with an error
        fi
    ...
    if [ "$SB_DELETE_OLD_LINK" == "t" ]; then
        rm -f $S_FP_LINK
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "55a54871-d925-4adb-b3bc-20d2e060b9e7"
        func_mmmv_wait_and_sync_t1
        if [ -e "$S_FP_LINK" ]; then
            echo ""
            echo ""
            ...
            echo ""
            echo "PWD==`pwd`"
            echo "GUID=='505f272d-bd97-41d4-939b-20d2e060b9e7'"
            echo ""
            echo ""
            exit 1 # exit with an error
        fi
    ...
    $S_CMD_LN
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "76aade15-c8fe-4304-92bc-20d2e060b9e7"
    func_mmmv_wait_and_sync_t1
    if [ ! -e "$S_FP_LINK" ]; then
        echo ""
        echo ""
        ...
        echo ""
        echo "PWD==`pwd`"
        echo "GUID=='9f163444-280c-4b07-829b-20d2e060b9e7'"
        echo ""
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_determine_operatingsystem_t1() {
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "8de18d38-7def-43fe-93bc-20d2e060b9e7"
    fi
...
func_mmmv_assert_Linux_or_BSD_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='fe958404-0a3e-45b6-829b-20d2e060b9e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" == "" ]; then
        ...
        echo ""
        echo "GUID=='1b741818-4ed9-456a-a29b-20d2e060b9e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
...
func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='42e63452-c2fa-4b93-b19b-20d2e060b9e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
            ...
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" != "f" ]; then
                ...
                if [ "$S_GUID_CANDIDATE" != "" ]; then
                    ...
                    func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1 \
                        "$SB_DISPLAY_NONERROR_FEEDBACK" \
                        "SB_DISPLAY_NONERROR_FEEDBACK" \
                        "a72ccf27-2f0d-4aa1-a5bc-20d2e060b9e7"
                fi
                SB_DISPLAY_NONERROR_FEEDBACK="f"
                #----------------------------------------
            fi
...
# RENESSAATOR_BLOCK_END
    ...
    if [ "$SB_DOMAIN_IS_REACHABLE" == "f" ]; then
        ...
        if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            echo "GUID=='79b9d72e-d2ef-4eec-919b-20d2e060b9e7'"
            if [ "$S_GUID_CANDIDATE" != "" ]; then
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
        fi
...
func_mmmv_assert_internet_connection_exists_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='56e126d5-eced-41ba-949b-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "cbedb532-a067-4d64-92bc-20d2e060b9e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_STUDY_INTERNET_AVAILABILITY" == "t" ]; then
        ...
        # The
        func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1 \
            "10a26912-48e9-49aa-a29c-20d2e060b9e7" \
            "$SB_DISPLAY_NONERROR_FEEDBACK"
        ...
        if [ "$SB_VERIFICATION_FAILED" == "t" ]; then
            ...
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
                ...
                func_mmmv_assert_sbvar_domain_t_f_t1 \
                    ...
                    "SB_VERIFICATION_FAILED" \
                    "40ab2504-d349-4bc0-8f9c-20d2e060b9e7"
            fi
        fi
        #----------------------------------------
    ...
    if [ "$SB_INTERNET_AVAILABLE" == "f" ]; then
        ...
        echo "GUID=='217ee744-5127-4b4c-b49b-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_GUI_applications_can_be_launched_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='2f808de4-cb7c-4f78-a39b-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_OK_TO_CACHE" "SB_OK_TO_CACHE" \
            "39089235-562e-4bee-948c-20d2e060b9e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_OK_TO_CACHE" == "t" ]; then
        if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_RESULT_CACHED" == "t" ]; then
            ...
            if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_GUI_IS_LAUNCHABLE" == "f" ]; then
                ...
                echo "Aborting script."
                echo "GUID=='24a3f945-97d9-4341-959b-20d2e060b9e7'"
                echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                echo ""
                #--------
                cd "$S_FP_ORIG"
    ...
    if [ "$SB_RUN_TEST" == "t" ]; then
        ...
        if [ "$S_TMP_0" != "124" ]; then
            ...
            echo "Aborting script."
            echo "GUID=='11ee1f35-5091-4f5e-849b-20d2e060b9e7'"
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_exc_initialize_wine_C_drive_if_needed_t1(){
    ...
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_DISPLAY_NONERROR_FEEDBACK" "SB_DISPLAY_NONERROR_FEEDBACK" \
            "a2f5afe7-08ff-4fd3-9f7c-20d2e060b9e7"
    fi
    #----------------------------------------------------------------------
    ...
    if [ ! -d "$S_FP_C" ]; then # for speed optimisation
        if [ ! -e "$S_FP_C" ]; then
            ...
            func_mmmv_assert_GUI_applications_can_be_launched_t1 \
                "55446919-bcbc-4110-a36c-20d2e060b9e7" \
            ...
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "9fa02e54-4e28-4af7-856c-20d2e060b9e7"
            func_mmmv_wait_and_sync_t1
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "445cb022-60a9-4d31-a26c-20d2e060b9e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            #--------------------------------------------------------------
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "1e35a5d1-e83a-47b5-b16c-20d2e060b9e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            #--------------------------------------------------------------
        fi
    fi
...
func_mmmv_uncompress_tar_dot_foo_and_enter_t2(){
    ...
    if [ "$SB_COMPRESSED_FILE_WRAPS_A_SINGLE_TARFILE" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "SB_COMPRESSED_FILE_WRAPS_A_SINGLE_TARFILE" \
            "040b6e28-7d97-4c65-b45c-20d2e060b9e7"
    ...
    if [ "$SB_COMPRESSION_TYPE_RECOGNIZED" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='0b67d34d-c4b7-41cc-819b-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_PARENT_FOLDER" \
        "ac267916-90c6-4872-a25c-20d2e060b9e7"
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "acbcb128-30d7-4497-915c-20d2e060b9e7"
    #----------------------------------------
    ...
    if [ "$SB_TAR_WRAPPED" == "t" ]; then
        ...
        if [ "$SB_COMPRESSIONLESS_TAR" == "f" ]; then
            func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
                "d97fd354-4a8b-4547-b45c-20d2e060b9e7"
            #--------
            if [ "$SB_MODE_ZIP" == "t" ]; then
                $S_CMD_NICE_PREFIX  zip --test $S_FP_COMPRESSED
                func_mmmv_assert_error_code_zero_t1 "$?" \
                    "b286d015-fdc4-4a66-814c-20d2e060b9e7"
            fi
            #--------
            $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "2806cc30-d7da-42b6-934c-20d2e060b9e7"
            func_mmmv_wait_and_sync_t1
        fi
        #----------------------------------------
        func_mmmv_assert_file_exists_t1 "$S_FP_TAR" \
            "45471ab1-806b-44d2-8d4c-20d2e060b9e7"
        $S_CMD_NICE_PREFIX tar -xf $S_FP_TAR
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "2602e335-2daf-4a71-b54c-20d2e060b9e7"
        func_mmmv_wait_and_sync_t1
        #----------------------------------------
    else
        #----------------------------------------
        ...
        #----------------------------------------
        func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
            "d2e79b4d-f7be-4865-b23c-20d2e060b9e7"
        #--------
        if [ "$SB_MODE_ZIP" == "t" ]; then
            $S_CMD_NICE_PREFIX  zip --test $S_FP_COMPRESSED
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "ad5afd24-40dd-4e97-b53c-20d2e060b9e7"
        fi
        #--------
        $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "479cd04b-1e1f-4252-913c-20d2e060b9e7"
        func_mmmv_wait_and_sync_t1
        #----------------------------------------
    fi
    #----------------------------------------
    func_mmmv_assert_folder_exists_t1 "$S_FP_NEXT_PARENT_FOLDER" \
        "46e3cb4f-3ec6-4fe1-a23c-20d2e060b9e7"
...
func_mmmv_s_owner_of_a_file_or_folder_t1(){
    ...
    if [ ! -e "$S_FP_IN" ]; then
        if [ -h "$S_FP_IN" ]; then
            func_mmmv_exc_exit_with_an_error_t2 \
                "63b3cd57-7cc5-4b08-832c-20d2e060b9e7" \
                "The $S_FP_IN is a broken symlink."
        else
            func_mmmv_exc_exit_with_an_error_t2 \
                "a5123e02-ae3d-4fa5-812c-20d2e060b9e7" \
...
func_mmmv_exc_block_device_ID_to_device_file_name_t1(){ #
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='c626312c-84c1-4c4c-959b-20d2e060b9e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_BLOCK_DEVICE_ID_SUBPART_CANDIDATE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='19efd2d4-bd03-4c95-8e9b-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='72cde744-b0fc-476b-a59b-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_FP_DEVICE_FILE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='20483f29-a97b-4489-918b-20d2e060b9e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_delete_folder_of_build_folders(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_FOLDER_OF_BUILD_FOLDERS" \
        "a4d7f62c-c3c8-4b48-832c-20d2e060b9e7" \
        "$SB_OPTIONAL_BAN_SYMLINKS"
    #------------------------------
    local S_FP_DELETABLE="$S_FP_ORIG/subject_to_deletion_0_$S_TIMESTAMP"
    if [ -e "$S_FP_DELETABLE" ]; then
        ...
        echo ""
        echo "GUID=='ce00e08b-bf2e-438d-b38b-20d2e060b9e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
        ...
        if [ -h "$S_FP_DELETABLE" ]; then # a broken symlink here
            ...
            echo "    S_FP_DELETABLE==\"$S_FP_DELETABLE\""
            echo ""
            echo "GUID=='477120ce-1876-423c-938b-20d2e060b9e7'"
            echo ""
            #------------------------------
            cd "$S_FP_ORIG"
            exit 1
    ...
    if [ "$S_TMP_0" != "" ]; then # for safety
        ...
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "71ea5b15-0a99-4809-822c-20d2e060b9e7"
        func_mmmv_wait_and_sync_t1
        ...
        echo ""
        echo "GUID=='bf5caa20-ff7b-430e-b18b-20d2e060b9e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
...
func_first_use_message(){
    ...
    echo "GUID=='05dbc11d-5221-4c5f-848b-20d2e060b9e7'"
    echo ""
    #------------------------------
    cd "$S_FP_ORIG"
    exit 1
...
func_display_help_message(){
    ...
    echo -e "is an absolute path of an\e[33m .ova \e[39mfile. The .ova file will be"
    echo -e "converted to a\e[33m .qcow2 \e[39mfile that the QEMU can load."
    echo -e "The .qcow2 is a virtual appliance disk image. In the case of AMD64 "
    echo -e "virtual appliances the QEMU MIGHT be started with Bash commands "
    echo -e "\e[36m"
    echo -e "    SI_PERCENTAGE_OF_A_SINLE_CPU_CORE=\"70\" "
    echo -e "    SI_AMOUNT_OF_VIRTUAL_APPLIANCE_RAM_IN_GiB=\"2048\" "
    echo -e "    cpulimit -l \$SI_PERCENTAGE_OF_A_SINLE_CPU_CORE -- qemu-system-x86_64 -m \$SI_AMOUNT_OF_VIRTUAL_APPLIANCE_RAM_IN_GiB -hda foo.qcow2"
    echo -e ""
    echo -e "    SI_N_OF_CPU_CORES=\"2\" "
    echo -e "    qemu-system-x86_64 -m \$SI_AMOUNT_OF_VIRTUAL_APPLIANCE_RAM_IN_GiB -smp \$SI_N_OF_CPU_CORES -hda foo.qcow2"
    echo -e "\e[39m"
    echo -e "Thank You for trying out this script. "
    echo ""
} # func_display_help_message
...
func_check_for_some_dependencies(){
    ...
    func_mmmv_assert_exists_on_path_t1 "wc" # as "word count"
    #------------------------------
    ...
    #----------------------------------------------------------------------
    func_mmmv_assert_exists_on_path_t1 "qemu-img"
    #----------------------------------------------------------------------
...
func_display_help_message_and_exit_if_needed(){
    ...
    if [ "$S_ARGV_0" == "" ]; then
        echo ""
        echo -e "\e[31mFile path candidate not found\e[39m."
        echo "GUID=='8211c934-36fd-4e85-858b-20d2e060b9e7'"
        func_display_help_message
        echo ""
        #--------
        cd "$S_FP_ORIG"
        ...
        if [ "$S_ARGV_1" != "" ]; then
            ...
            echo "GUID=='d439c646-bad2-48cc-a38b-20d2e060b9e7'"
            func_display_help_message
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_initialize_configuration_by_initializing_global_variables(){
    ...
    func_mmmv_init_s_timestamp_if_not_inited_t1
    #----------------------------------------------------------------------
    S_FP_FOLDER_OF_BUILD_FOLDERS="$S_FP_ORIG/temporary_build_folder"
    S_FP_BUILD_FOLDER="$S_FP_FOLDER_OF_BUILD_FOLDERS/tmp_$S_TIMESTAMP"
    S_FP_INSTALLATION_FOLDER="$S_FP_ORIG/conversion_result_in_QEMU_format"
    # The .ova files are supposedly renamed .tar files.
    #------------------------------
    local S_TMP_0="`echo \"$S_ARGV_0\" | $S_CMD_GNU_SED -e 's/[.][.]//g' `"
    if [ "$S_TMP_0" != "$S_ARGV_0" ]; then
        echo ""
        echo -e "\e[31mDoubledots are not allowed in the file path\e[39m."
        echo ""
        echo "    $S_ARGV_0"
        echo ""
        echo "Aborting script."
        echo "GUID=='5c0f2121-a0bb-4f68-848b-20d2e060b9e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
    ...
    #------------------------------
    S_FP_OVA_SRC="` echo \"$S_ARGV_0\" | $S_CMD_GNU_SED -e 's/[\\/]\\+/\\//g' | $S_CMD_GNU_SED -e 's/\\([\\/][.][\\/]\\)\\+/\\//g' | $S_CMD_GNU_SED -e 's/\\([\\/][.][\\/]\\)\\+/\\//g' | replace \"./\" \"$S_FP_ORIG/\" `"
    S_CMD_PREFIX_NICE="nice -n 12 "
    #----------------------------------------------------------------------
} # func_initialize_configuration_by_initializing_global_variables
...
func_main(){
    ...
    func_display_help_message_and_exit_if_needed
    #------------------------------
    func_mmmv_assert_folder_exists_t1 "$S_FP_ORIG" \
        "670aa25a-539c-4c57-951c-20d2e060b9e7"
    cd $S_FP_ORIG # just a superficial access test
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "4acfb301-3eac-4ec5-a31c-20d2e060b9e7"
    #------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_OVA_SRC" \
        "17bdd812-3a26-49ff-851c-20d2e060b9e7"
    # Bash testline:
    #     echo "aaaa.txT2" | rev | sed -e 's/[.].\+$//g' | rev | tr '[:upper:]' '[:lower:]' | tr -d '.'
    local S_FN_OVACANDIDATE="`basename $S_FP_OVA_SRC`"
    local S_OVACANDIDATE_EXT_CASE_SENSITIVE="`echo  \"$S_FN_OVACANDIDATE\" | \
                                              rev | \
                                              $S_CMD_GNU_SED -e 's/[.].\\+\$//g' | \
                                              rev | tr -d '.' `"
    local S_OVACANDIDATE_EXT_DOWNCASE="`echo \"$S_OVACANDIDATE_EXT_CASE_SENSITIVE\" | \
                                        tr '[:upper:]' '[:lower:]' `"
    if [ "$S_OVACANDIDATE_EXT_DOWNCASE" != "ova" ]; then
        echo ""
        echo -e "\e[31mWrong file extension.\e[39m"
        echo -e "Found a file "
        echo ""
        echo "    $S_FP_OVA_SRC"
        echo ""
        echo -e "with an extension of \"\e[31m$S_OVACANDIDATE_EXT_CASE_SENSITIVE\e[39m\", but "
        echo -e "the expected file extension is \"\e[33mova\e[39m\"."
        echo "GUID=='dbf47afd-4d4c-4962-928b-20d2e060b9e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
    fi
    local S_OVACANDIDATE_EXTENSIONLESS="`echo \"$S_FN_OVACANDIDATE\" | \
                                         $S_CMD_GNU_SED -e 's/[.]'\"$S_OVACANDIDATE_EXT_CASE_SENSITIVE\"'\$//g' `"
    local S_TMP_0="/$S_FN_OVACANDIDATE"
    local S_FP_OVA_WORKING_COPY="$S_FP_BUILD_FOLDER$S_TMP_0"
    S_TMP_0="/$S_OVACANDIDATE_EXTENSIONLESS"
    local S_TMP_1=".qcow2"
    local S_FP_QCOW2_IN_BUILD_FOLDER="$S_FP_BUILD_FOLDER$S_TMP_0$S_TMP_1"
    local S_FP_QCOW2_IN_INSTALLATION_FOLDER="$S_FP_INSTALLATION_FOLDER$S_TMP_0$S_TMP_1"
    #------------------------------
    func_mmmv_create_folder_t1 "$S_FP_BUILD_FOLDER"
    cd $S_FP_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "17ce32d2-e456-4437-921c-20d2e060b9e7"
    #------------------------------
    # func_mmmv_assert_file_exists_t1 "$S_FP_OVA_SRC" \
    #     "5fa05bb2-b735-455f-af0c-20d2e060b9e7"
    $S_CMD_PREFIX_NICE cp -f $S_FP_OVA_SRC $S_FP_OVA_WORKING_COPY
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "ae392419-a5d9-4579-b40c-20d2e060b9e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    $S_CMD_PREFIX_NICE tar -xf $S_FP_OVA_WORKING_COPY
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "b6d1695f-837d-4448-940c-20d2e060b9e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    # A single .ova file can contain more than one .vmdk file.
    S_TMP_0="` ls -1 | grep -E '[.]vmdk\$' | wc -l `"
    if [ "$S_TMP_0" != "1" ]; then
        echo ""
        if [ "$S_TMP_0" == "0" ]; then
            echo -e "\e[31mFailed to find .vmdk files form the .ova file\e[39m"
            echo ""
            echo "    $S_FP_OVA_SRC"
            echo ""
            echo "Aborting script."
            echo "GUID=='307aa521-d423-4482-938b-20d2e060b9e7'"
        else
            echo -e "\e[31mThis script has not been designed to handle \e[39m"
            echo -e "\e[31m.ova files that contain multiple .vmdk files\e[39m."
            echo ""
            echo "    $S_FP_OVA_SRC"
            echo ""
            echo "Aborting script."
            echo "GUID=='901ae64f-6f8f-491a-918b-20d2e060b9e7'"
        fi
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
    ...
    #------------------------------
    # The .vmdk file extention free name part might NOT
    # match with the .ova file extention free name part.
    local S_FP_VMDK="$S_FP_BUILD_FOLDER/` ls -1 | grep -E '[.]vmdk\$' `"
    func_mmmv_assert_file_exists_t1 "$S_FP_VMDK" \
        "1c5a96d2-8853-4597-b20c-20d2e060b9e7"
    #------------------------------
    $S_CMD_PREFIX_NICE qemu-img convert -O qcow2 $S_FP_VMDK $S_FP_QCOW2_IN_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "4eb45d41-ed30-432c-b2fb-20d2e060b9e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    local S_TMP_0=""
    local SB_OPTIONAL_BAN_SYMLINKS="t"
    if [ -e "$S_FP_INSTALLATION_FOLDER" ]; then
        S_TMP_0="`echo \"$S_FP_INSTALLATION_FOLDER\" | grep -E '/conversion_result_in_QEMU_format$' `"
        if [ "$S_TMP_0" != "" ]; then # for safety
            func_mmmv_assert_folder_exists_t1 "$S_FP_INSTALLATION_FOLDER" \
                "5ff3489c-9fcf-4169-b5fb-20d2e060b9e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            ...
            echo ""
            echo "GUID=='12e0b7d2-518f-41fc-a18b-20d2e060b9e7'"
            echo ""
            #------------------------------
            cd "$S_FP_ORIG"
            exit 1
    ...
    #------------------------------
    $S_CMD_PREFIX_NICE mv $S_FP_QCOW2_IN_BUILD_FOLDER $S_FP_QCOW2_IN_INSTALLATION_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "44090652-7f36-4092-84fb-20d2e060b9e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    func_delete_folder_of_build_folders
    #------------------------------
    echo ""
    echo "The location of the converted disk image is:"
    echo -e "\e[92m    $S_FP_QCOW2_IN_INSTALLATION_FOLDER \e[39m"
    echo ""
    echo "Thank You for trying out this script. "
    echo "GUID=='46269b71-f9ec-4120-958b-20d2e060b9e7'"
    echo ""
    #------------------------------
} # func_main
func_main
...
exit 0
#==========================================================================
# S_VERSION_OF_THIS_FILE="7e886c36-8f31-4005-a3fb-20d2e060b9e7"
#==========================================================================
